document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchAndFillForm() {
        try {
            ...
            document.getElementById('cep').value = dadosPerfil.cep || '';
    ...
    // Verifica o ID do cargo do utilizador e redireciona se necessário
    // Verifica o ID do cargo do utilizador e redireciona se necessário
    async function verificaIdVoltar() {
        const idCargo = localStorage.getItem('idCargo');

        // Converte o valor do localStorage para um número, pois ele é armazenado como string
        ...
        if (idCargoNumero === 1) {
            ...
            // Você pode adicionar um redirecionamento para uma página de erro ou de login, por exemplo
    ...
    formEditarPerfil.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await fetch(`/api/usuario/editar/${data.id}`, {
                method: 'PUT',
